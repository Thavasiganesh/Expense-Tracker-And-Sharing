...
if (strlen($_SESSION['ETASaid']==0)) {
    ...
    if(isset($_GET['delid'])) {
        $row=intval($_GET['delid']);
        $query=mysqli_query($con,"delete from userstbl where UserID='$row'");
        if($query) {
            echo "<script>alert('User successfully removed');</script>";
            echo "<script>window.location.href='reg_users.php'</script>";
        } else {
                                    ...
                                    <tbody>
                                        ...
                                        while ($row=mysqli_fetch_array($ret)) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="user_profile.php?editid=<?php echo $row['UserID']; ?>" class="edit-link">Edit</a> |
                                                    <a href="reg_users.php?delid=<?php echo $row['UserID']; ?>">Delete</a> |
